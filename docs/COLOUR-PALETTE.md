...
A palette is *counted*, not averaged. The **Colour Palette** flow (`art.palette`)
takes an image in and gives you the colours it actually uses most — not the
colours you get by averaging it, which is how palettes end up as five shades of
mud.
...
Every pixel is counted. The counts are sorted. The commonest colours are taken in
...
So there is a **minimum distance**. A colour closer than that to one already
chosen is not a new entry: it joins that entry's *group*, and the group keeps its
own tally. One group is one mode of the image, and the palette is one colour
...
| --- | --- |
| Colours | How many entries the palette has. |
| Minimum distance | How far apart two entries must look. Anything closer joins a group. |
| Temperature | How far each entry may wander from its group's commonest colour. |
| Seed | Same seed, same palette. Only matters above temperature 0. |
| Colour precision | How finely colours are rounded together before counting. |
| Transparency floor | Pixels this transparent are not counted at all. |
| Drop groups under | Leaves out a colour that barely appears. |
...
each seen once or twice. The commonest colour is then whichever one happened to
repeat, which is noise. So colours are rounded before they are counted —
**5 bits a channel** by default, which is 32 levels each.

Coarse enough for a wall to be one colour; fine enough that a palette entry is
still a colour you can see in the picture. Pure white stays `#ffffff` rather than
...
## Temperature, and why it cannot invent a colour
...
It never leaves the group, which is the point: a palette colour is always a
colour the image actually contains. Rerolling the seed moves it somewhere else
...
rather than resized. Resizing interpolates, and interpolation invents colours
...
| --- | --- | --- |
| Palette | `palette.json` | Each colour as hex and RGB, with the share of the image it accounts for, its pixel count, how many counted colours were in its group, and that group's commonest colour. |
| Report | `report.md` | The palette as a table, plus what was counted, what was grouped, and why. |

Shares always add up to the whole image: a colour with no group near enough joins
the nearest one anyway rather than being dropped, so the numbers describe the
picture completely.

## When it will not give you what you asked for

Ask for eight colours at a distance of 30 from a two-tone drawing and you get
...
whole point of the minimum distance is that near-duplicates are not colours.

Lower the distance, or ask for fewer.

## Pinning

Any entry can be **pinned** to a colour you choose. A pinned entry is used as it
is, whatever the settings do, and pins are held by position so they survive
changing the count or the distance. For a brand colour, or when the count found
